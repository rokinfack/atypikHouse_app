      ...
      <div class="habitat-info-container">
        ...
        <div class="habitat-info-content">
          ...
          <div class="habitat-reservation">
            <p><span class="price">{{ difDate * parseInt(getHabitat.price) }} €</span> pour <span>{{ difDate
              }} nuits</span></p>
          </div>
        </div>
      </div>
...
export default defineComponent({
  name: "ReservationAuthentication",
  components: {},
  emits: ["navigate"],
  computed: {
    ...mapState(useHabitatReservation, ["getHabitat"])
  },
  data() {
    return {
      auth: {
  ...
  setup(_, { emit }) {
    ...
    return { startDate, endDate, difDate, useAuth, v$: useVuelidate(), habitatReservation, navigation, emit };
  },
  methods: {
    getImage(cover) {
    ...
    async submit() {
      ...
      this.isLoading = true;
      let value = await this.useAuth.login(this.auth.email, this.auth.password);

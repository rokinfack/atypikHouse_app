<template>
  <div class="admin-reservation-page">
    ...
    <div class="admin-reservation-content">
      <data-table v-bind="{ columns }" v-bind:resources="getResources" v-bind:per-page-options="[5,10,20,30,50]"
                  @refetch="equipment.updateUrl(baseUrl+$event);url=baseUrl+$event"
  ...
  <BaseModal v-bind:show="openModal">
    <div class="modal-content">
      <div class="header">
        <h2>Création d'équipment</h2>
        <i class="fa-solid fa-close cursor-pointer" @click="openModal = false"></i>
      </div>
      <form class="form-modal-content" @submit.prevent="handleEquipment">
        <div class="form-groups" :class="{'error':v$?.equipmentForm?.name?.$errors?.length > 0}">
          ...
          <input type="text" class="form-control" placeholder="Nom de l'équipement"
                 ...
                 @blur="v$?.equipmentForm.name?.$touch"
          >
          <p v-if="v$.equipmentForm?.name?.$error" class="form-error">{{ v$.equipmentForm?.name?.$errors[0]?.$message
            }}</p>
        </div>
...
export default defineComponent({
  ...
  setup() {
    const equipment = useEquipmentStores();

    return { equipment,v$: useVuelidate()  };
  },
  computed: {
    ...mapState(useEquipmentStores, ["getResources","getPaginationData"])
  },
  data() {
    return {
      url: "equipment",
  ...
  watch: {
    url: {
      ...
      handler: async function() {
        await this.equipment.dataTableGet();
        console.log("fabien");
  ...
  methods: {
    openModalAction() {
      ...
      this.equipment.updateSelectedData(null);
      this.equipmentForm = {
        name: ""
      };
    },
    edit(event) {
      ...
      this.equipment.updateSelectedData(event);
      this.openModal = true;
    ...
    async handleEquipment() {
      ...
      if (this.selectedData) {
        await this.equipment.updateEquipment(this.selectedData.id, this.equipmentForm);
      } else {
        await this.equipment.createEquipments(this.equipmentForm);
      }

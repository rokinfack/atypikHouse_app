...
<!--        </button>-->
    ...
    <div class="admin-reservation-content">
      <data-table v-bind="{ columns }" v-bind:resources="getResources" v-bind:per-page-options="[5,10,20,30,50]"
                  @refetch="userStores.updateUrl(baseUrl+$event);url=baseUrl+$event"
        ...
        <template v-slot:cell="{ resource, column }">
          ...
          <div v-if="column.attribute === 'operation'" class="space-x-3">
            <button class="btn btn-primary rounded" @click="edit(resource)">
              Edit
            </button>
            <button class="btn bg-red-500 rounded" @click="edit(resource)">
              Blocker
            </button>
          </div>
        </template>
  ...
  <BaseModal v-bind:show="openModal">
    <div class="modal-content">
      <div class="header">
        <h2>Création d'utiliateur</h2>
        <i class="fa-solid fa-close cursor-pointer" @click="openModal = false"></i>
      </div>
      <form class="form-modal-content" @submit.prevent="handleClient">
        <div class="form-groups" :class="{'error':v$?.form?.name?.$errors?.length > 0}">
          <label for="localisation" class="label">Nom de l'équipement</label>
...
import { useClientUsersStores } from "stores/client-users.stores";
...
export default defineComponent({
  ...
  setup() {
    const userStores = useClientUsersStores();

    return { userStores, v$: useVuelidate() };
  },
  computed: {
    ...mapState(useClientUsersStores, ["getResources", "getPaginationData"])
  },
  data() {
    return {
      url: "users",
      ...
      response: null,
      columns: [
        { name: "FirstName", attribute: "firstName", sortable: "firstName" },
        { name: "LastName", attribute: "lastName", sortable: "lastName" },
  ...
  watch: {
    url: {
      ...
      handler: async function() {
        await this.userStores.dataTableGet();
        console.log("fabien");
  ...
  methods: {
    openModalAction() {
      ...
      this.userStores.updateSelectedData(null);
      this.form = {
        name: ""
      };
    },
    edit(event) {
      ...
      this.userStores.updateSelectedData(event);
      this.openModal = true;
      // this.equipmentForm.name = this.selectedData.name;
    },
    async handleClient() {
      ...
      if (this.selectedData) {
        await this.userStores.updateUser(this.selectedData.id, this.equipmentForm);
      } else {
        await this.userStores.createUser(this.equipmentForm);
      }

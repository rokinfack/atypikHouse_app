    ...
    <div class="admin-blog-content-new">
      <div class="admin-blog-form-content">
        <form @submit.prevent="handleSubmit">
          ...
          <div class="form-groups" :class="{'error':v$?.blogForm?.order_affichage?.$errors.length > 0}">
            ...
            <select name="disposition" id="disposition" class="form-control"
                    v-model="blogForm.order_affichage" @blur="v$.blogForm.order_affichage?.$touch"
            >
              <option v-for="i in parseInt(3)" v-bind:key="i" :value="i">{{ i }}</option>
            </select>
            <p v-if="v$.blogForm?.order_affichage?.$error" class="form-error">
              {{ v$.blogForm?.order_affichage?.$errors[0].$message }}</p>
          </div>
          ...
          <div class="button-container">
            <button class="btn btn-primary w-full block rounded" type="submit">Mettre a jour</button>
          </div>
        </form>
      </div>
    </div>
...
export default defineComponent({
  ...
  methods: {
    async handleSubmit() {
      ...
      await this.blogActivities.updateBlogActivities(this.blogForm);
    },
  ...
  async mounted() {
    await this.blogActivities.getCurrentBlogActivities(this.$route.params.id);
    this.blogForm.slug = this.getSelectedBlog.slug;
    this.blogForm.title = this.getSelectedBlog.title;
    // this.blogForm.coverImage  = this.getSelectedBlog.coverImage;
    this.blogForm.description = this.getSelectedBlog.description;
    this.blogForm.htmlContent = this.getSelectedBlog.htmlContent;
    this.blogForm.order_affichage = this.getSelectedBlog.order_affichage;
    this.blogForm.isPublished = this.getSelectedBlog.isPublished;

    if (this.getSelectedBlog.coverImage){
      this.blogForm.coverImage = this.getSelectedBlog.coverImage["@id"]
      this.mediaStores.medias = [this.getSelectedBlog.coverImage]
    }
  }
});
</script>

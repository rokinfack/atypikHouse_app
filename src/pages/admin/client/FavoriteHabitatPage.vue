<template>
  <div class="admin-habitat-page">
    <div class="breadcrumb-container">
      <Breadcrumb />
      <!--      <div>-->
      <!--        <button class="btn btn-primary rounded w-52 ">New Habitat</button>-->
      <!--      </div>-->
    </div>
    <div class="admin-habitat-header">
      <div class="header">
        <h2>Veuillez retrouver ici tout vos habitats préférés</h2>
      </div>
    ...
    <div class="admin-habitat-content">
      <div v-if="getLikes && getLikes['hydra:member']?.length >0" class="space-y-3">
        <div class="card-habitat" v-for="like in getLikes['hydra:member']" v-bind:key="like">
          <div class="card-image">
            <img :src="getImage(like.habitat.coverImage)" class="image" alt="">
            ...
            <div class="card-description-container">
              <div class="info-content">
                <div class="name-container">
                  <span class="location">{{ like.habitat.location }}</span>
                  <span class="name">{{ like.habitat.name }}</span>
                </div>
                <div class="price-container">
                  <span class="start">à partir de</span>
                  <span class="price">
                {{ like.habitat.price }}€ /nuit
              </span>
                </div>
              </div>
              ...
              <div class="description-content">
                <p>{{ like.habitat.description }}</p>
              </div>
            </div>
            <div class="card-close-content">
              <span class="hover:bg-neutral-200 rounded-full p-2 " @click="deleteLike(like)"><i
                class="fa fa-close p-2 text-sm text-gray-600 "></i></span>
            </div>
          </div>
        </div>
      </div>
      <div v-else class="flex w-full h-52 justify-center items-center">
        <h3 class="text-xl text-blue-800">Aucun habitat aimé pour le moment</h3>
      </div>
    </div>
  </div>
</template>
...
import { useHabitatFavorite } from "stores/habitat-favorite.stores";
import { mapState } from "pinia";

export default defineComponent({
  name: "FavoriteHabitatPage",
  components: {
    Breadcrumb
  }, setup() {
    const reservationFavorite = useHabitatFavorite();
    reservationFavorite.getLikesHabitats();

    return { reservationFavorite };
  },
  computed: {
    ...mapState(useHabitatFavorite, ["getLikes"])
  },
  methods: {
    async deleteLike(event) {
      await this.reservationFavorite.deleteLike(event);
    },
    getImage(cover) {

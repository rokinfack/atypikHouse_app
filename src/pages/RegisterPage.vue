        ...
        <form class="mx-9" @submit.prevent="submitForm">
          ...
          <div class="form-content">
            <div class="form-inline">
              <div class="form-groups" :class="{'error':v$.auth?.lastName.$errors.length > 0}">
                <label for="lastName" class="label">Nom</label>
                <input type="text" class="form-control" name="lastName" id="lastName"
                       placeholder="Nom" @blur="v$.auth?.lastName.$touch" v-model="auth.lastName">
                <p v-if="v$.auth?.lastName.$error" class="form-error">{{ v$.auth?.lastName.$errors[0].$message }}</p>
              </div>
              <div class="form-groups" :class="{'error':v$.auth?.firstName.$errors.length > 0}">
                <label for="firstName" class="label">Prénoms</label>
                <input type="text" class="form-control" name="firstName" id="firstName"
                       placeholder="Prénom" @blur="v$.auth?.firstName.$touch" v-model="auth.firstName">
                <p v-if="v$.auth?.firstName.$error" class="form-error">{{ v$.auth?.firstName.$errors[0].$message }}</p>
              </div>
            </div>
            <div class="form-groups" :class="{'error':v$.auth?.email.$errors.length > 0}">
              <label for="email" class="label">Email</label>
              <input type="email" name="email" id="email" class="form-control" placeholder="Email"
                     @blur="v$.auth?.email.$touch" v-model="auth.email">
              <p v-if="v$.auth?.email.$error" class="form-error">{{ v$.auth?.email.$errors[0].$message }}</p>
            </div>

            <div class="form-groups" :class="{'error':v$.auth?.birthDay.$errors.length > 0}">
              <label for="birthDay" class="label">Date de naissance</label>
              <input type="date" class="form-control" name="birthDay" id="birthDay"
                     placeholder="BirthDay" @blur="v$.auth.birthDay.$touch" v-model="auth.birthDay">
              <p v-if="v$.auth?.birthDay.$error" class="form-error">{{ v$.auth?.birthDay.$errors[0].$message }}</p>
            </div>

            <div class="form-groups" :class="{'error':v$.auth?.password.$errors.length > 0}">
              <label for="password" class="label">Mot de passe</label>
              <input type="password" class="form-control" placeholder="Mot de passe"
                     @blur="v$.auth?.password.$touch" v-model="auth.password">
              <p v-if="v$.auth?.password.$error" class="form-error">{{ v$.auth?.password.$errors[0].$message }}</p>
            </div>

            <div class="form-groups" :class="{'error':v$.auth?.password_confirmation.$errors.length > 0}">
              <label for="passwordConfirmation" class="label">Confirmer mot de passe</label>
              <input type="password" class="form-control" placeholder="Confirmer mot de passe"
                     @blur="v$.auth?.password_confirmation.$touch" v-model="auth.password_confirmation">
              <p v-if="v$.auth?.password_confirmation.$error" class="form-error">
                {{ v$.auth?.password_confirmation.$errors[0].$message }}</p>
            </div>

            <div class="form-groups col flex items-center"
                 :class="{'error':v$.auth?.isCheckedTerms.$errors.length > 0}">
              <input type="checkbox" class="check-box" name="checkbox" id="checkbox"
                     @blur="v$.auth?.isCheckedTerms.$touch" v-model="auth.isCheckedTerms" />
              <label for="checkbox" class="check-box-label">Accepter les conditions d'utilisation <a href="#/">link</a>
              </label>
              <p v-if="v$.auth?.isCheckedTerms.$error" class="form-error block w-full">
                {{ v$.auth?.isCheckedTerms.$errors[0].$message }}
              </p>
            </div>
            <div class="form-groups col flex items-center">
              <input type="checkbox" class="check-box" name="newsLetters" id="newsLetters"
                     v-model="auth.isNewsletters" />
              <label for="newsLetters" class="check-box-label">Accepter les newsletters
              </label>
            </div>
...
import { email, minLength, sameAs, required } from "@vuelidate/validators";
...
export default defineComponent({
  ...
  validations() {
    return {
      auth: {
        ...
        password_confirmation: {
          required,
          sameAsPassword: sameAs(this.auth.password),
          sameAsRef: sameAs(this.auth.password)
        },
        isCheckedTerms: {
          required,
          sameAsRef: sameAs(true)
        }
      }
    };
  },
  methods: {
    async submitForm() {
      ...
      this.isLoading = true;
      let users = await this.authStores.register(this.auth);

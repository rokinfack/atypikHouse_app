...
export const useCategoryPropertyStores = defineStore("category-property", {
  state: () => ({
    properties: [],
  ...
  actions: {
    ...
    async getCategoriesProperty() {
      const categories = await api.get("categories?pagination=false");
      this.categories = categories.data["hydra:member"]
      console.log(this.categories);
    },
    async updateProperty(id, data) {
      try {
        let newCategory = await api.put(`properties/${id}`, data);
    ...
    async getData() {
      try {
        const data = await api.get("properties?order[createdAt]=desc");

        this.properties = data.data["hydra:member"];
    ...
    async createProperties(data) {
      try {
        let property = await api.post("properties", data);
        this.categories.push(property.data);
